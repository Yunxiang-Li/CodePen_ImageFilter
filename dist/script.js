/* initialize four global variables for one image and four composite image to null, also initialize the canvas. 
*/
...
/* Function: chooseFile
  Parameter:None
  Return:None
  Does:after user clicks the button,load that file.
  change it to a SimpleImage file and draw it to the canvas.
  store this SimpleImage file in five seperate variables.
*/
function chooseFile(){
  alert('one file chose');
  var imgfile = document.getElementById('fileinput');
  oriImage = new SimpleImage(imgfile);
  greyImage = new SimpleImage(imgfile);
  redImage = new SimpleImage(imgfile);
  rainbowImage = new SimpleImage(imgfile);
  blurImage = new SimpleImage(imgfile);
  oriImage.drawTo(canvas); 
}

/* Function: imageIsLoaded
  Parameter:None
  Return:Boolean True or False
  Does:check whether the user upload the image and the image is
  completely uploaded.If yes, return True, else output error message   and return False.
*/  
function imageIsLoaded(){
  if ((oriImage == null)||(!oriImage.complete())){
...
/* Function: doReset
  Parameter:None
  Return:None
  Does:if the origin image is uploaded completely,
  draw it to the canvas.
*/  
function doReset(){
  alert('reset the image');
  if ( imageIsLoaded(oriImage) ){
  oriImage.drawTo(canvas);    
  }
}
...
/* Function: doGrayscale
  ...
  Return:None
  Does:if the origin image is uploaded completely,
  grayscale it and draw it to the canvas.
*/  
function doGrayscale(){
  alert('gray scale the image');
    if ( imageIsLoaded(greyImage) ) {     // check if image is loaded
    filterGray();	                      // function performs the grayscale work
    greyImage.drawTo(canvas);	          // display image
  }
}
...
/* Function: filterGray
  ...
  Return:None
  Does:if the origin image is uploaded completely,
  red it and draw it to the canvas.
*/  
function filterGray(){
  //for each pixel
  for (var pixel of greyImage.values()){
    //calculate its average RGB value;
    var avg = (pixel.getRed() + pixel.getGreen() + pixel.getBlue() )/3;
    //set each to this average value.
    pixel.setRed(avg);
    pixel.setGreen(avg);
    pixel.setBlue(avg);
  }
...
/* Function: filterRed
  Parameter:None
  Return:None
  Does:set each pixel's RGB value through specific rules.
*/  
function filterRed(){
  //for each pixel
  ...
  if (avg < 128){
      ...
      //if average more than or equals to 128
      else{
        pixel.setRed(255);
...
function filterBlur() {
  ...
  for(var pixel of blurImage.values()) {
   var x = pixel.getX();
   var y = pixel.getY();
  //set pixels in the original image to random numbers// 
   var x1 = x - Math.round(Math.random()) * 5;
   var y1 = y - Math.round(Math.random()) * 5;
   //set specific rules
   if(x1 < 0){
     x1 = -0.5 * x1;
   }
   if(y1 < 0){
    ...
    if( (Math.random() < 0.5) && (x1 < (canvas.width -1)) && (y1 < (canvas.height -1)) ) {
     blurImage.setPixel(x,y,oriImage.getPixel(x1,y1));
    }
  }
}
...
/* Function: doRainbow
  ...
  Return:None
  Does:if the origin image is uploaded completely,
  rainbow it and draw it to the canvas.
*/  
function doRainbow(){
  alert('rainbow the image');
    if ( imageIsLoaded(rainbowImage) ) {     // check if image is loaded
    filterRainbow();	                      // function performs the grayscale work
    rainbowImage.drawTo(canvas);	          // display image
  }
}
...
function filterRainbow(){
  ...
  for (var pixel of rainbowImage.values()){
    ...
    var avg = (pixel.getRed() + pixel.getGreen() + pixel.getBlue() )/3; 
    //if the pixel is in top 1/7 position
    if((pixel.getY() <= (rainbowImage.getHeight() /7)) && (pixel.getY() > 0 ) ){
      if(avg < 128){
        pixel.setRed(2 * avg);
        pixel.setGreen(0);
        pixel.setBlue(0);
    ...
    //if the pixel is in top 1/7 to 2/7 position
    else if((pixel.getY() <= (2 * rainbowImage.getHeight() /7)) && (pixel.getY() > (rainbowImage.getHeight() /7) ) ){
            if(avg < 128){
        pixel.setRed(2 * avg);
        ...
        //if the pixel is in top 2/7 to 3/7 position
        else if((pixel.getY() <= (3 * rainbowImage.getHeight() /7)) && (pixel.getY() > (2 * rainbowImage.getHeight() /7) ) ){
            if(avg < 128){
        pixel.setRed(2 * avg);
    ...
    //if the pixel is in top 3/7 to 4/7 position
     else if((pixel.getY() <= (4 * rainbowImage.getHeight() /7)) && (pixel.getY() > (3 * rainbowImage.getHeight() /7) ) ){
            if(avg < 128){
        pixel.setRed(0);
        pixel.setGreen(2 * avg);
        pixel.setBlue(0);
        ...
        //if the pixel is in top 4/7 to 5/7 position
         else if((pixel.getY() <= (5 * rainbowImage.getHeight() /7)) && (pixel.getY() > (4 * rainbowImage.getHeight() /7) ) ){
            if(avg < 128){
        pixel.setRed(0);
        pixel.setGreen(0);
        pixel.setBlue(2 * avg);
        ...
        pixel.setBlue(255);
           ...
           //if the pixel is in top 5/7 to 6/7 position
           else if((pixel.getY() <= (6 * rainbowImage.getHeight() /7)) && (pixel.getY() > (5 * rainbowImage.getHeight() /7) ) ){
            if(avg < 128){
    ...
    //if the pixel is in bottom 1/7 position
    else{
            if(avg < 128){
